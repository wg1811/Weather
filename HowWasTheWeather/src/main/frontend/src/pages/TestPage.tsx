...
const TestPage: React.FC = () => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const token = await forecastApi.login("test1234@test.com", "1234");
  ...
  const getVisibleDailyForecast = (): DailyForecastEntry[] => {
    ...
    const dailyEntries: DailyForecastEntry[] = forecastData.dailyDTO.time.map(
      (time, index) => ({
        time,
  ...
  const getNext24HoursForecast = (): HourlyForecastEntry[] => {
    ...
    const hourlyEntries: HourlyForecastEntry[] = forecastData.hourlyDTO.time
      .slice(0, 24)
      .map((time, index) => ({
        time,
        temperature_2m: forecastData.hourlyDTO.temperature_2m[index],
        apparent_temperature:
          forecastData.hourlyDTO.apparent_temperature[index],
