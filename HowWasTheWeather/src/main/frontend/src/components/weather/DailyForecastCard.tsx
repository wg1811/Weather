...
import { DailyForecastEntry, HourlyForecastEntry, getWeatherDetails } from "../../types/weather";
...
const DailyForecastCard: React.FC<DailyForecastProps> = ({ dailyData, hourlyData }) => {
  ...
  const getHourlyForecastForSelectedDate = (dayTime: string): HourlyForecastEntry[] => {
    ...
    console.log("Selected Date:", selectedDate, "Filtered Hourly Data:", filteredData);
    return filteredData;
  };
    ...
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="space-y-4">
        {getVisibleDailyForecast().map((day, idx) => (
          <div key={idx}>
            <div
              onClick={() => setSelectedDay(selectedDay === day.time ? null : day.time)}
              className={`flex items-center justify-between border-b last:border-b-0 p-4 cursor-pointer transition-colors duration-200 rounded-md ${
                ...
                : "bg-white " + (idx === 0 ? "border-blue-100" : "border-gray-100")
              } hover:bg-blue-100`}
            >
                ...
                <div className="flex flex-col items-end mr-6">
                  ...
                  <div className="flex items-center">
                    ...
                    <span
                      ...
                      style={getWindDirectionStyle(day.wind_direction_10m_dominant)}
                    >
                      ↑
                    </span>
            ...
            {selectedDay === day.time && (
              <div className="mt-4 overflow-hidden transition-all duration-300 ease-in-out">
                <HourlyForecastCard hourlyData={getHourlyForecastForSelectedDate(day.time)} />
              </div>
            )}
          </div>
        ))}
